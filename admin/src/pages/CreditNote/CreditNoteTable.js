  ...
  const sendCreditNoteEmail = async () => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/send-credit-note-email/${selectedCreditNote.id}`,
        {
          method: "POST",
          headers: {
    ...
    <React.Fragment>
      ...
      <Modal isOpen={emailModal} toggle={closeEmailModal}>
        ...
        <ModalBody>
          <div className="mb-3">
            <Label>
              Credit Note #: {selectedCreditNote?.credit_note_number}
            </Label>
          </div>
          <div className="mb-3">
                      ...
                      {customers.map((creditNote) => (
                        <Tr key={creditNote.id}>
                          ...
                          <Td>
                            ...
                            {creditNote.created_by.last_name}
                          </Td>
                          ...
                          <Td>
                            <div className="d-flex gap-2">
                              <Button
                                color="warning"
                                onClick={() => handleViewTemplate(creditNote)}
                              >
                                Generate Template
                              </Button>
                              <Button
                                color="info"
                                onClick={() => openEmailModal(creditNote)}
                              >
                                Send Email
                              </Button>
                            </div>
