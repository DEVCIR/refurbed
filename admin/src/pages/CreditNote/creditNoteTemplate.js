...
export default function CreditNoteTemplate(creditNote, onBackClick) {
    ...
    const [items, setItems] = useState([
        { id: 1, description: "Front and rear brake cables", unitPrice: 100.00, quantity: 1, amount: 100.00 },
        { id: 2, description: "New set of pedal arms", unitPrice: 25.00, quantity: 2, amount: 50.00 },
        { id: 3, description: "Labor 3hrs", unitPrice: 15.00, quantity: 3, amount: 45.00 }
    ]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`http://localhost:8000/api/credit-notes-item/?credit_note_id=${creditNote.creditNote.id}`);
                const result = await response.json();

                
                const fetchedItems = result.data.data.map(item => ({
                    id: item.id,
                    description: `${item.inventory.variant.product.brand.brand_name} ${item.inventory.variant.product.model_name}` || '',
                ...
                setCreditInfo({
                    ...
                    creditNoteDate: new Date(result.data.data[0].credit_note.issue_date).toLocaleDateString(),
                    purchaseOrderNumber: result.data.data[0].credit_note.rma.order.order_number, // Assuming this is the PO number
                    dueDate: new Date(result.data.data[0].credit_note.updated_at).toLocaleDateString() // Example for due date
                });
                setCustomer({
                    name: result.data.data[0].credit_note.customer.user.name || '',
                    email: result.data.data[0].credit_note.customer.user.email || '',
                    phone: result.data.data[0].credit_note.customer.user.phone_number || '',
    ...
    const addNewRow = () => {
        ...
        setItems([...items, { id: newId, description: "", unitPrice: 0, quantity: 0, amount: 0 }]);
    };
                                ...
                                <tr>
                                    ...
                                    <td style={styles.infoTableValue}>
                                        <input
                                            ...
                                            onChange={(e) => setCreditInfo({ ...creditInfo, creditNoteNumber: e.target.value })}
                                            style={styles.inputNoBorder}
                                        />
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td style={styles.infoTableValue}>
                                        <input
                                            ...
                                            onChange={(e) => setCreditInfo({ ...creditInfo, creditNoteDate: e.target.value })}
                                            style={styles.inputNoBorder}
                                        />
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td style={styles.infoTableValue}>
                                        <input
                                            ...
                                            onChange={(e) => setCreditInfo({ ...creditInfo, purchaseOrderNumber: e.target.value })}
                                            style={styles.inputNoBorder}
                                        />
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td style={styles.infoTableValue}>
                                        <input
                                            ...
                                            onChange={(e) => setCreditInfo({ ...creditInfo, dueDate: e.target.value })}
                                            style={styles.inputNoBorder}
                                        />
                                    </td>
                                </tr>
                                ...
                                <tr key={item.id}>
                                    ...
                                    <td style={styles.tableCell}>
                                        <input
                                            ...
                                            value={item.amount}
                                            readOnly
                                            style={{ ...styles.tableCellInput, backgroundColor: '#f9f9f9' }}
                                        />
                                    </td>
