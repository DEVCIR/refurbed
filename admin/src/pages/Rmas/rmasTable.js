...
const RmasTable = (props) => {
  ...
  const fetchRmaData = async (page = 1) => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/rmas?page=${page}`);
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/rmas/${rmaToDelete.id}`, {
        method: "DELETE",
        headers: {
  ...
  return (
    <React.Fragment>
          ...
          <Card>
            <CardBody>
              ...
              >
                ...
                <Button color="primary" onClick={props.onAddRmasClick}>
      ...
      <Modal isOpen={deleteModal} toggle={() => setDeleteModal(false)}>
        ...
        <ModalBody>
          Are you sure you want to delete RMA {rmaToDelete?.rma_number}? This
          will also delete all related items and cannot be undone.
        </ModalBody>
        <ModalFooter>
