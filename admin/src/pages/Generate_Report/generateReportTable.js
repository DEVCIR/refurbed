...
const GenerateReportTable = (props) => {
  document.title =
    "Generated Reports | Lexa - Responsive Bootstrap 5 Admin Dashboard";
  ...
  const fetchReportData = async (page = 1) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/generated-reports?page=${page}`,
      );
  ...
  const handleDelete = async (reportId) => {
    try {
      if (window.confirm("Are you sure you want to delete this report?")) {
        const response = await fetch(
          `${API_BASE_URL}/generated-reports/${reportId}`,
          {
            method: "DELETE",
            headers: {
        ...
        if (response.ok) {
          ...
          alert(
            `Error deleting report: ${errorData.message || "Unknown error"}`,
          );
        }
      }
    } catch (error) {
          ...
          <Card>
            <CardBody>
              ...
              >
                ...
                <Button color="primary" onClick={props.onAddReportClick}>
