...
const ReportTemplateTable = (props) => {
  document.title =
    "Report Templates | Lexa - Responsive Bootstrap 5 Admin Dashboard";
  ...
  const fetchTemplateData = async (page = 1) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/report-templates?page=${page}`,
      );
    ...
    try {
      if (window.confirm("Are you sure you want to delete this template?")) {
        ...
        if (response.ok) {
          ...
          alert(
            `Error deleting template: ${errorData.message || "Unknown error"}`,
          );
        }
      }
    } catch (error) {
          ...
          <Card>
            <CardBody>
              ...
              >
                ...
                <Button color="primary" onClick={props.onAddTemplateClick}>
