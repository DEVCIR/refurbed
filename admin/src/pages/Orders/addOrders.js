    ...
    const fetchInventory = async () => {
      try {
        ...
        if (
          ...
          const activeInventory = result.data.data.filter(
            (item) => item.is_active && item.variant?.product?.is_active,
          );
  ...
  const checkVoucherCode = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/voucher?voucher_code=${voucherCode}`,
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const selectedCustomer = customers.find(
        (c) => c.user.name === formData.customerName,
      );
      if (!selectedCustomer) {
      ...
      for (const product of selectedProducts) {
        const orderItemData = {
          ...
          discount_amount:
            product.discount * (productQuantities[product.value] || 1),
          total_price:
            product.price * (productQuantities[product.value] || 1) -
            product.discount * (productQuantities[product.value] || 1),
        };
        ...
        if (!orderItemResponse.ok) {
          ...
          throw new Error(
            orderItemResult.message || "Failed to create order items",
          );
        }
      }
      ...
      if (!deliveryNoteResponse.ok) {
        ...
        throw new Error(
          deliveryNoteResult.message || "Failed to create delivery note",
        );
      }
      ...
      for (const product of selectedProducts) {
        ...
        const deliveryNoteItemResponse = await fetch(
        ...
        if (!deliveryNoteItemResponse.ok) {
          ...
          throw new Error(
            deliveryNoteItemResult.message ||
              "Failed to create delivery note items",
          );
        }
      }
                  ...
                  <Row className="mb-3">
                    ...
                    <div className="col-md-10">
                      <input
                        ...
                        value={
                          product.discount *
                          (productQuantities[product.value] || 1)
                        }
                        readOnly
                      />
                    </div>
                        ...
                        <button
                          ...
                          onClick={() =>
                            handleQuantityChange(
                              product.value,
                              (productQuantities[product.value] || 1) - 1,
                            )
                          }
                          disabled={
                            (productQuantities[product.value] || 1) <= 1
                ...
                <div className="col-md-8">
                  ...
                  {voucherStatus === true && voucherDetails && (
                    <div className="text-success mt-1">
                      Voucher applied: {voucherDetails.voucher_code} (
                      {voucherDetails.voucher_discount}
                  ...
                  {voucherStatus === false && (
                    <div className="text-danger mt-1">Invalid voucher code</div>
                  )}
