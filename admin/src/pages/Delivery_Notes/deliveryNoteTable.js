...
const DeliveryNoteTable = (props) => {
  document.title =
    "Delivery Notes | Lexa - Responsive Bootstrap 5 Admin Dashboard";
  ...
  const fetchDeliveryNoteData = async (page = 1) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/delivery-notes?page=${page}`,
      );
  ...
  const handleDelete = async (noteId) => {
    if (
      window.confirm(
        "Are you sure you want to delete this delivery note and all its items?",
      )
    ) {
      try {
        const response = await fetch(
          `${API_BASE_URL}/delivery-notes/${noteId}`,
          {
            method: "DELETE",
          },
        );
          ...
          <Card>
            <CardBody>
              ...
              >
                ...
                <Button color="primary" onClick={props.onAddDeliveryNoteClick}>
                      ...
                      {deliveryNoteData.length > 0 ? (
                        deliveryNoteData.map((note) => (
                          <tr key={note.id}>
                            ...
                            <td>
                              {displayField(note.order?.customer?.user?.name)}
                            </td>
                        ...
                        <tr>
                          <td colSpan="9" className="text-center">
                            No delivery note data available
                          </td>
                        </tr>
                      )}
                    </tbody>
