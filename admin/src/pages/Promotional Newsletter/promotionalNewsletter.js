...
export default function PromotionalNewsletter() {
  ...
  useEffect(() => {
    const fetchNewsletters = async () => {
      try {
        const response = await fetch('http://127.0.0.1:8000/api/newsletter-promotional');
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        ...
        const promoResponse = await fetch('http://127.0.0.1:8000/api/newsletter-promotional/count');
        if (!promoResponse.ok) {
        ...
        const subResponse = await fetch('http://127.0.0.1:8000/api/newsletter/count');
        if (!subResponse.ok) {
...
const handleSend = async (schedule = false) => {
  ...
  try {
    ...
    const newsletterResponse = await fetch('http://127.0.0.1:8000/api/newsletter-promotional', {
      method: 'POST',
      headers: {
    ...
    for (const email of emails) {
      const sendResponse = await fetch('http://localhost:8000/api/send-promotional-email', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    if (!schedule) {
      ...
      await fetch(`http://127.0.0.1:8000/api/newsletter-promotional/${newsletterData.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    showNotification(error.message || 'An error occurred while sending the newsletter.', 'error');
  } finally {
    setIsSending(false);
  }
};
        ...
        <tbody>
          {newsletters && newsletters.length > 0 ? (
            newsletters.map(newsletter => {
              ...
              let recipients = [];
              try {
                recipients = newsletter.recipients ? JSON.parse(newsletter.recipients) : [];
              } catch (e) {
              ...
              return (
                <tr key={newsletter.id} style={styles.tableRow}>
                  <td style={styles.tableCell}>{newsletter.subject_line}</td>
      ...
      case 'analytics':
        return (
          <div style={{
            ...
            <div style={{...styles.statsCard, background: 'linear-gradient(135deg, #eafdf7 0%, #ffffff 100%)'}}>
              <h3 style={{...styles.statsTitle, color: '#047857'}}>Subscriber Emails</h3>
              <div style={{...styles.statsContent, position: 'relative'}}>
                ...
                <div style={{...styles.statsMetric, position: 'relative', zIndex: 1}}>
                  <div style={{
                    ...styles.statsValue, 
                    color: '#10b981', 
                    fontSize: '3.5rem',
            ...
            <div style={{...styles.statsCard, background: 'linear-gradient(135deg, #f5f7ff 0%, #ffffff 100%)'}}>
              <h3 style={{...styles.statsTitle, color: '#4338ca'}}>Promotional Emails</h3>
              <div style={{...styles.statsContent, position: 'relative'}}>
                <div style={{
                  position: 'absolute',
                  width: '150px',
